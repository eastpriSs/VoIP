...
PBXAuthCredits::PBXAuthCredits(const QString &clientId, const QString &clientSecret, const QString &server) {
    ...
    QRegularExpression clientIdRegex("^[a-zA-Z0-9_-]{3,50}$");
    if (!clientIdRegex.match(clientId).hasMatch()) {
        throw std::invalid_argument("Некорректный Client ID. Допускаются только латинские буквы, цифры, дефисы и подчеркивания (от 3 до 50 символов).");
    }
    ...
    QRegularExpression serverRegex(R"(^https:\/\/[a-zA-Z0-9.-]+(?::[0-9]+)?(\/.*)?$)");
